!---------------------------------------------------------------------------------!
! pmlib client: vortex_turbulence
!---------------------------------------------------------------------------------!
PROGRAM vortex_turbulence
...
!---------------------------------------------------------------------------------!
    SELECT CASE ( TRIM(flowcase) )
      CASE ('taylor_green')
        CALL flowcases_taylor_green(topo_all%cuboid,mesh,ierr)
      CASE ('turbulence')
        CALL flowcases_turbulence( patch,topo_all,mesh,ierr, &
...
!---------------------------------------------------------------------------------!
      IF(itimestage .EQ. 1)THEN
        SELECT CASE ( TRIM(flowcase) )
          CASE ('turbulence')
!            CALL diagnostics_spectral(patch,topo_all,mesh,ierr)
            CALL diagnostics_energy(topo_all%cuboid,mesh,ierr)
            IF(vort_max .LT. 1.0E-6_MK) abort = .TRUE.
          CASE ('taylor_green')
!            CALL diagnostics_spectral(patch,topo_all,mesh,ierr)
            CALL diagnostics_energy(topo_all%cuboid,mesh,ierr)
            IF(vort_max .LT. 1.0E-6_MK) abort = .TRUE.
