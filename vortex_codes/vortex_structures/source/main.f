!---------------------------------------------------------------------------------!
! pmlib client: vortex_structures
!---------------------------------------------------------------------------------!
PROGRAM vortex_structures
...
!---------------------------------------------------------------------------------!
    SELECT CASE ( TRIM(flowcase) )
      CASE ('ring')
        CALL flowcases_ring(patch,topo_all,mesh,part,ierr)
      CASE ('gauss')
        CALL flowcases_gauss(topo_all%cuboid,mesh,ierr)
      CASE ('ringfit')
        CALL flowcases_ringfit(topo_all%cuboid,mesh,ierr)
      CASE ('bump')
        CALL flowcases_bump(topo_all%cuboid,mesh,ierr)
      CASE ('collision')
        CALL flowcases_collision(topo_all%cuboid,mesh,ierr, &
                                  & perturbe=0.05_MK)
      CASE ('helix')
        CALL flowcases_helix(patch,topo_all,mesh,ierr)
      CASE ('knot')
        CALL flowcases_knot(patch,topo_all,mesh,ierr)
      CASE DEFAULT
        ierr = -1
...
!---------------------------------------------------------------------------------!
      IF(itimestage .EQ. 1)THEN
        SELECT CASE ( TRIM(flowcase) )
          CASE ('ring')
            CALL diagnostics_ring(topo_all%cuboid,part,ierr)
          CASE ('gauss')
            CALL diagnostics_ring(topo_all%cuboid,part,ierr)
          CASE ('ringfit')
            CALL diagnostics_ring(topo_all%cuboid,part,ierr)
          CASE ('bump')
            CALL diagnostics_bump(topo_all%cuboid,part,ierr)
          CASE ('collision')
            CALL diagnostics_collision(topo_all%cuboid,part,ierr)
          CASE ('helix')
            CALL diagnostics_helix(topo_all%cuboid,part,ierr)
          CASE ('knot')
            CALL diagnostics_knot(topo_all%cuboid,part,ierr)
        END SELECT
      END IF
