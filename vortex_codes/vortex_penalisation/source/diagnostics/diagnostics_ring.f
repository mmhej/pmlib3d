...
!---------------------------------------------------------------------------------!
SUBROUTINE diagnostics_ring(topo,part,ierr)
...
! Reduce and communicate
  ...
  CALL MPI_ALLREDUCE( sum_vort_cen,vort_centroid,ndim,mpi_prec_real, &
                    &  MPI_SUM,comm,ierr )
  ...
  CALL MPI_REDUCE( sum_ang_imp,ang_impulse,ndim,mpi_prec_real, &
                 & MPI_SUM,0,comm,ierr  )
...
!---------------------------------------------------------------------------------!
! Calculate 1st radius (ring radius) and ring circulation
!---------------------------------------------------------------------------------!
  DO i = 1,topo(rank)%npart
! Calculate the vortex ring radius using the enstrophy
    sum_vort_rad = sum_vort_rad + & 
          ...
          & + part%vort(2,i) * part%vort(2,i) &
          & + part%vort(3,i) * part%vort(3,i) &
          & ) * SQRT( (part%pos(2,i) - vort_centroid(2))**2 &
          & + (part%pos(3,i) - vort_centroid(3))**2 )*dx(1)*dx(2)*dx(3)


! Calculate circulation (project vorticity onto the azimuthal unit normal vector)
    sum_circ = sum_circ - (part%pos(3,i) - vort_centroid(3)) & 
        & /( (part%pos(2,i) - vort_centroid(2))**2 &
        &  + (part%pos(3,i) - vort_centroid(3))**2 ) * part%vort(2,i) &
        &  + (part%pos(2,i) - vort_centroid(2)) & 
        & /( (part%pos(2,i) - vort_centroid(2))**2 &
        &  + (part%pos(3,i) - vort_centroid(3))**2 ) * part%vort(3,i)

  END DO

! Reduce and communicate
  CALL MPI_ALLREDUCE( sum_vort_rad,vort_radius,1,mpi_prec_real, &
                    &  MPI_SUM,comm,ierr )
  CALL MPI_REDUCE( sum_circ,circulation,1,mpi_prec_real,MPI_SUM,0,comm,ierr  )

! Normalise
  vort_radius = vort_radius/enstrophy
  circulation = circulation*dx(1)*dx(2)*dx(3)/(2.0_MK*PI)

!---------------------------------------------------------------------------------!
! Calculate second radius (core radius)
!---------------------------------------------------------------------------------!
  DO i = 1,topo(rank)%npart
! Calculate the vortex core radius using the enstrophy
    sum_vort_core = sum_vort_core + & 
          & ( part%vort(1,i) * part%vort(1,i) &
          & + part%vort(2,i) * part%vort(2,i) &
          & + part%vort(3,i) * part%vort(3,i) &
          & ) * SQRT( ( SQRT( (part%pos(2,i) - vort_centroid(2))**2          &
          & + (part%pos(3,i) - vort_centroid(3))**2 ) - vort_radius )**2     &
          & + (part%pos(1,i) - vort_centroid(1))**2 )                        &
          & *dx(1)*dx(2)*dx(3)
  END DO

! Reduce and communicate
  CALL MPI_ALLREDUCE( sum_vort_core,vort_core,1,mpi_prec_real, &
                    &  MPI_SUM,comm,ierr )
! Normalise
  vort_core = vort_core/enstrophy
...
!---------------------------------------------------------------------------------!
      ...
      diag(11,idiag) = circulation
      diag(12,idiag) = vort_centroid(1)
      diag(13,idiag) = vort_radius
      diag(14,idiag) = vort_core
...
END SUBROUTINE diagnostics_ring
