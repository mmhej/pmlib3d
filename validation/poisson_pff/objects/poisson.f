...
!---------------------------------------------------------------------------------!
    IF( patch%bound_cond(1) .EQ. 1 .AND. &
      & patch%bound_cond(2) .EQ. 0 .AND. &
      & patch%bound_cond(3) .EQ. 0)THEN
      CALL poisson_setup_pff(patch,topo_all,mesh,ierr)
    ELSE
  ...
  DO k = 1-nghost(5),ncell(3)+nghost(6)
    ...
    DO j = 1-nghost(3),ncell(2)+nghost(4)
      ...
      DO i = 1-nghost(1),ncell(1)+nghost(2)
        ...
        IF ( SQRT(py**2 + pz**2) .LT. r0) THEN
          mesh%vort(1,i,j,k) = ( EXP(c*(py**2 + pz**2)/(py**2 + pz**2 - 1.0_MK)) &
                             & * SIN(PI*px) &
                             & * ( 1.0_MK*PI**2*(py**8 + pz**8) &
                             & - 4.0_MK*PI**2*(py**6 + pz**6) &
                             & + 6.0_MK*PI**2*(py**4 + pz**4) &
                             & - 4.0_MK*PI**2*(py**2 + pz**2) &
                             & + 4.0_MK*PI**2* py**6 * pz**2 &
                             & + 4.0_MK*PI**2* py**2 * pz**6 &
                             & + 6.0_MK*PI**2* py**4 * pz**4 &
                             & - 12.0_MK*PI**2* py**4 * pz**2 &
                             & - 12.0_MK*PI**2* py**2 * pz**4 &
                             & + 12.0_MK*PI**2* py**2 * pz**2 &
                             & - 8.0_MK *c* py**2 * pz**2 &
                             & - 4.0_MK *c*(py**4 + pz**4) &
                             & - 4.0_MK *c**2*(py**2 + pz**2) &
                             & + 1.0_MK*PI**2 &
                             & + 4.0_MK *c ) &
                             & )/( (py**2 + pz**2 - 1.0_MK)**4 )
...
! velocity check
          ...
          vel(2) = - 2.0_MK*c*pz*EXP(c*(py**2 + pz**2)/(py**2 + pz**2 - 1.0_MK)) &
                 & * SIN(PI*px)/(py**2 + pz**2 - 1.0_MK)**2

          vel(3) = 2.0_MK*c*py*EXP(c*(py**2 + pz**2)/(py**2 + pz**2 - 1.0_MK)) &
                 & * SIN(PI*px)/(py**2 + pz**2 - 1.0_MK)**2
...
! vorticity check
          ...
          vort(1) = ( EXP(c*(py**2 + pz**2)/(py**2 + pz**2 - 1.0_MK)) &
                  & * SIN(PI*px) &
                  & * ( 1.0_MK*PI**2*(py**8 + pz**8) &
                  & - 4.0_MK*PI**2*(py**6 + pz**6) &
                  & + 6.0_MK*PI**2*(py**4 + pz**4) &
