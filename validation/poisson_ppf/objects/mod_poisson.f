...
# 1 "sources/poisson/poisson_setup_ppf.f" 1
!---------------------------------------------------------------------------------!
! poisson_setup.f
...
! Calculate wave number in x-direction (FFT-shifted)
            ...
            IF( SQRT( ki*ki + kj*kj ) .NE. 0.0_MK )THEN
              pkernel_real(1,i,j,k) = 0.5_MK * c_1_2pi &
                         & * EXP( - 2.0_MK * PI * SQRT( ki*ki + kj*kj ) * r ) &
                         & /( SQRT( ki*ki + kj*kj ) ) / ( dx(1) * dx(2) )

! pkernel_real(1,i,j,k) = 1.0_MK/(4.0_MK * PI**2 * SQRT( ki*ki + kj*kj + kk*kk) )
            END IF
...
! Calculate wave number in x-direction (FFT-shifted)
            ...
            ELSE IF( rho * s .LT. 700.0_MK )THEN
              pkernel_real(1,i,j,k) = 0.25_MK/(2.0_MK * PI * SQRT( ki*ki + kj*kj ) ) * &
                  & ( ( 1.0_MK - ERF( c_1_sqrt2 * (s - rho) ) ) &
                  & * EXP( - s*rho ) &
                  & + ( 1.0_MK - ERF( c_1_sqrt2 * (s + rho) ) ) &
                  & * EXP( s*rho ) &
                  & ) / dx(1)
...
! Calculate wave number in x-direction (FFT-shifted)
            ...
            ELSE IF( rho * s .LT. 700.0_MK )THEN
              pkernel_real(1,i,j,k) = ( 0.25_MK/(2.0_MK * PI * SQRT( ki*ki + kj*kj ) ) * &
                  ...
                  & + ( 1.0_MK - ERF( c_1_sqrt2 * (s + rho) ) ) &
                  & * EXP( s*rho ) &
                  & ) + eps * c_sqrt_2_pi * 0.25_MK &
                  & * EXP( - 0.5_MK * (s**2 + rho**2) ) &
                  & ) / dx(1)
            END IF
...
! Calculate wave number in x-direction (FFT-shifted)
            ...
            ELSE IF( rho * s .LT. 700.0_MK )THEN
              pkernel_real(1,i,j,k) = ( 0.25_MK/(2.0_MK * PI * SQRT( ki*ki + kj*kj ) ) * &
                  ...
                  & + 1.0_MK/12.0_MK * s**2 &
                  & - 1.0_MK/8.0_MK * rho**2 &
                  & - 1.0_MK/48.0_MK * s**2*rho**2 &
                  & + 1.0_MK/96.0_MK * (s**4 + rho**4) &
                  & ) &
                  & * EXP( - 0.5_MK * (s**2 + rho**2) ) &
                  & ) / dx(1)
            END IF
...
! Calculate wave number in x-direction (FFT-shifted)
            ...
            ELSE IF( rho * s .LT. 700.0_MK )THEN
              pkernel_real(1,i,j,k) = ( 0.25_MK/(2.0_MK * PI * SQRT( ki*ki + kj*kj ) ) * &
                  ...
                  & ) + eps * c_sqrt_2_pi * &
                  & ( 93.0_MK/256.0_MK &
                  & + 73.0_MK/768.0_MK * s**2 &
                  & - 47.0_MK/256.0_MK * rho**2 &
                  & - 17.0_MK/384.0_MK * s**2*rho**2 &
                  & + 11.0_MK/768.0_MK * s**4 &
                  & + 23.0_MK/768.0_MK * rho**4 &
                  & + 1.0_MK/256.0_MK * s**2*rho**4 &
                  & - 1.0_MK/256.0_MK * s**4*rho**2 &
                  & + 1.0_MK/768.0_MK * (s**6 - rho**6) &
                  & ) &
                  & * EXP( - 0.5_MK * (s**2 + rho**2) ) &
                  & ) / dx(1)
            END IF
