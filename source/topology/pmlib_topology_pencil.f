!---------------------------------------------------------------------------------!
! pmlib_topology_pencil.f
!   version: 3D multi-core
!---------------------------------------------------------------------------------!
SUBROUTINE pmlib_topology_pencil(patch,topo,pencil_dir,ierr,fextend,nextend)
...
!  Local variables
!---------------------------------------------------------------------------------!
  CHARACTER(LEN=21)                            :: caller = 'pmlib_topology_pencil'
  INTEGER                                      :: iproc
...
!---------------------------------------------------------------------------------!
  ...
  DO i = 1,pmlib_ndim
    IF( patch%bound_cond(i) .EQ. 0 )THEN
      xmin(i)  = xmin(i) - dx(i)*patch%nghost
      xmax(i)  = xmax(i) + dx(i)*patch%nghost
...
END SUBROUTINE pmlib_topology_pencil
